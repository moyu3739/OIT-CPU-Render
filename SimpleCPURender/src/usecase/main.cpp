...
void PipelinedRenderCustom2(Application& app, float time_limit) {
    ...
    DoubleFrameBufferManager frame_buffer_manager(app.width, app.height, render_thread_num, true);
    Displayer displayer;
...
void PipelinedRenderCustom3(Application& app, float time_limit) {
    ...
    auto pipeline = app.InitPipeline();
    TripleFrameBufferManager frame_buffer_manager(app.width, app.height, render_thread_num, true);
    Displayer displayer;
    ...
    while(1){
        ...
        std::thread thread_clear([&](){
            ...
            float start_clear = tm.ReadTimer();
            load_buffer->Clear(glm::vec3(1.0f));
            duration_clear = tm.ReadTimer() - start_clear;
            total_clear_time += duration_clear;
        });
